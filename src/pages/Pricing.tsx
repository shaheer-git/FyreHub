        ...
        <div className='max-w-7xl w-full'>
          ...
          <div className='text-center mb-12 md:mb-16 animate-fade-in'>
            ...
            <div className='inline-flex items-center bg-[#2a2630] rounded-full p-1 border border-[#AFFC41] border-opacity-30'>
              <button
                ...
                className={`px-6 md:px-8 py-2 md:py-3 rounded-full font-semibold transition-all ${billingCycle === 'monthly'
              ...
              <button
                ...
                className={`px-6 md:px-8 py-2 md:py-3 rounded-full font-semibold transition-all ${billingCycle === 'yearly'
                    ? 'bg-[#AFFC41] text-[#252525]'
                    : 'text-[#AFFC41]'
                  }`}
              >
          ...
          <div className='grid grid-cols-1 md:grid-cols-3 gap-6 lg:gap-8 mb-16'>
            {plans.map((plan, idx) => (
              <div
                key={idx}
                className={`relative rounded-3xl transition-all duration-300 transform hover:scale-105 ${plan.highlighted
              ...
              >
                ...
                {/* Plan Name */}
                <h3 className={`text-2xl md:text-3xl font-bold mb-2 ${plan.highlighted ? 'text-[#252525]' : 'text-[#AFFC41]'
                  }`}>
                  {plan.name}
                </h3>

                {/* Description */}
                <p className={`text-sm md:text-base mb-6 ${plan.highlighted ? 'text-[#252525] opacity-90' : 'text-[#AFFC41] opacity-80'
                  }`}>
                  {plan.description}
                </p>
                ...
                <div className='mb-8'>
                  <div className='flex items-baseline gap-1'>
                    <span className={`text-4xl md:text-5xl font-bold ${plan.highlighted ? 'text-[#252525]' : 'text-[#AFFC41]'
                      }`}>
                      ₹{getPrice(plan.monthlyPrice).toLocaleString()}
                    </span>
                    <span className={`text-sm ${plan.highlighted ? 'text-[#252525]' : 'text-[#AFFC41]'
                      } opacity-75`}>
                      / {billingCycle === 'monthly' ? 'month' : 'year'}
                    </span>
                  </div>
                  {billingCycle === 'yearly' && (
                    <p className={`text-xs mt-2 ${plan.highlighted ? 'text-[#252525]' : 'text-[#AFFC41]'
                      }`}>
                ...
                <button
                  ...
                  className={`w-full py-3 md:py-4 rounded-lg font-bold mb-8 transition-all transform hover:scale-105 ${plan.highlighted
                ...
                <div className='space-y-3'>
                  {plan.features.map((feature, featureIdx) => (
                    <div key={featureIdx} className='flex items-start gap-3'>
                      <svg
                        className={`w-5 h-5 mt-0.5 flex-shrink-0 ${plan.highlighted ? 'text-[#252525]' : 'text-[#AFFC41]'
                          }`}
                        fill='currentColor'
                        viewBox='0 0 20 20'
                      >
                      ...
                      <span className={`text-sm md:text-base ${plan.highlighted ? 'text-[#252525]' : 'text-[#AFFC41]'
                        }`}>
                        {feature}
                      </span>
                    </div>
